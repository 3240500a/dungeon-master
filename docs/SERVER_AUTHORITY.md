...
| Экономика (магазин/экип/распределение/бинды/пояс) | `economy/townActions.ts` | Room.handleCmd (`buy/sell/equip/unequip/allocAttr/respec/respecPassives/respecSkills/allocPassive/allocSkill/moveBelt/bind/useConsumable/drop`) | ✅ | клиент НЕ мутирует save. **`allocPassive`** — вход из ЛЮБОГО из 4 входов дерева мастерства (класс-гейт снят, Ф6), остальное по смежности (переходы дают край соседней ветви). **`allocSkill`** = вложить очко в древо скилов (смежность + класс-ветка по `classId`). **`respecSkills`** = сброс древа скилов: возврат всех очков скиллов, комиссия `balance.skillRespecCostPerPoint`×очки, бинды сброшенных скиллов чистятся. **`respecPassives`** = сброс мастерства: возвращает очки (Σ рангов, чистит и осиротевшие после регена узлы), вложенное золото НЕ возвращает; комиссия = `balance.passiveRespecCostPct` × вложенного золота (растёт с прокачкой) |
...
| Конфиг игры (редактор ↔ игра) | `config/registry.ts reload/snapshot` | `GET /api/config` (клиент+редактор); `POST /api/dev/config` (→ БД `config_overrides`, «тест»), `POST /api/dev/config-file` (→ **пишет data/*.json** + оверрайд, «применить везде»), `DELETE /api/dev/config/:key` (dev-only) → `rebuildConfig` | ✅ | **единая серверная истина**: дефолты (data/*.json) + персистентные оверрайды БД (переживают рестарт). **Две кнопки редактора:** «Применить (тест)» = оверрайд в БД (локально, НЕ в git); «Применить везде (в файл)» = запись в `data/<key>.json` (попадёт в git/деплой) + оверрайд для живости. Имя файла = `key.replace('.','-')+.json`; путь от сервера `../../shared/src/config/data` (dev-layout). Клиент/редактор грузят эффективный конфиг с сервера (localStorage выпилен). Запись гейтится `NODE_ENV!=='production'` (анти-чит) |
