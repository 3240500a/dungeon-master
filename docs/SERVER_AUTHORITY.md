...
|---|---|---|---|---|
| Движение/коллизии | `world/movement.ts`, `session.ts` | Room.step → tick | ✅ | клиент — чистый вид |
| Бой/скиллы/снаряды | `formulas/combat`, `world/combat`, `session.ts` | tick | ✅ | |
...
| Реконнект (грейс-окно) | `session.addPlayer(spawnAt)`, `economy/death` | `Room.removePlayer`→грейс (тело убирается, комната на паузе если пусто), `Room.reconnect` (та же точка / начало этажа), `RoomManager.graceByChar` (роут по charId), `expireGrace`/`finalizeDisconnectedAsDead` | ✅ | дисконнект НЕ уничтожает комнату: возврат в подземелье (не в город) в течение `balance.reconnectGraceSec` (деф. 1ч). Пусто→пауза+час→все погибли (штраф); пати-вайп→отключённые тоже погибли. Анти-эксплойт: нельзя дисконнектом уйти в город. Проверено live (resume+expiry) |
