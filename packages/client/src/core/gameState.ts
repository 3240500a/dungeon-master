...
export class GameState {
  save: SaveState;
  /** Текущая глубина активного подземелья (не сохраняется). Город = 0; забег начинается с этажа 1 (без «старта»). */
  depth = 0;
  /** Текущая область: город или подземелье (не сохраняется). Различать город/этаж НАДО по этому полю, а не по depth
   *  (у старта забега depth=0, как и у города — depth один их не отличает). */
  area: 'town' | 'dungeon' = 'town';
