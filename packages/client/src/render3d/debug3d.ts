...
export function mountDebug(scene: THREE.Scene, camera: THREE.Camera, canvas: HTMLCanvasElement, root: HTMLElement, opts: { onMonKinematic?: (on: boolean) => void; onLowRes?: (on: boolean) => void } = {}): Debug3d {
  let on = false;
  ...
  panel.append(layerRow, physRow, lowRow, infoEl);
