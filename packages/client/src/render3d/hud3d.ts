...
import { effectiveLevel, startChallenge, challengeAtFloor, activeToggleInfos, DEBUFF_ICON, type DebuffKind } from '@dm/shared';
...
export function mountHud3d(app: App): Hud3d {
  ...
  const debuffLine = mkLine(116, 'font-size:16px;color:#e8907c');
  const auraLine = mkLine(138, 'font-size:13px;color:#e39a3c');
  ...
  return {
    update(): void {
      ...
      // Дебаффы на игроке (иконка + стаки) и активные ауры/стойки.
      debuffLine.textContent = (Object.keys(st.debuffs) as DebuffKind[])
        .map((k) => (st.debuffs[k] ? `${DEBUFF_ICON[k]}${st.debuffs[k]!.stacks}` : '')).filter(Boolean).join('  ');
