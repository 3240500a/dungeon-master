...
// ── rare-names ───────────────────────────────────────────────────────────────
/** Тема слова (стихия/физика) для гашения противоречий с аффиксами. Слово подходит предмету, если
 * делит с ним хотя бы одну тему; пустой список тем = универсальное (подходит всем). */
export const rareThemeSchema = z.enum(['fire', 'cold', 'lightning', 'poison', 'physical']);
/** Слово rare-имени: текст `t` + список тем `themes` (мультивыбор в редакторе; пусто = универсальное). */
 ...
 * Имя предмета = имя базы + титул («Ручной топор Пепел древних»). У слова — темы (стихии/физика),
